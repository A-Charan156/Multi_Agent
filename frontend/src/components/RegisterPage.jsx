...
const RegisterPage = () => {
  ...
  return <div className="auth-page"><div className="cyber-grid" /><div className="orb orb-1" /><div className="orb orb-2" /><button className="theme-toggle-btn" onClick={toggleTheme} aria-label="Toggle theme">{isDark ? "\u2600\uFE0F" : "\u{1F319}"}</button><div className="auth-container animate-fadeInUp"><div className="auth-logo"><div className="logo-icon">⚡</div><span className="font-display" style={{ fontSize: "1.1rem", letterSpacing: "0.12em", color: "var(--neon-cyan)" }}>
            DEBATE ARENA
          </span></div><div className="auth-card glass-card"><div className="auth-header"><h1 className="auth-title">Join the Arena</h1><p className="auth-subtitle">Create your account to witness AI debates.</p></div><form onSubmit={handleSubmit} className="auth-form" id="register-form"><div className="form-group"><label className="form-label" htmlFor="reg-username">Username</label><input
    ref={usernameRef}
    id="reg-username"
    type="text"
    ...
    maxLength={30}
    required
  /></div><div className="form-group"><label className="form-label" htmlFor="reg-email">Email Address</label><input
    id="reg-email"
    type="email"
    className="input-field"
    ...
    autoComplete="email"
    required
  /></div><div className="form-group"><label className="form-label" htmlFor="reg-password">Password</label><input
    id="reg-password"
    type="password"
    className="input-field"
    ...
    required
  />{strength && <div className="password-strength animate-fadeIn"><div className="strength-bars">{[1, 2, 3, 4].map((i) => <div
    key={i}
  ...
  />)}</div><span style={{ fontSize: "0.8rem", color: strength.color }}>{strength.label}</span></div>}</div><div className="form-group"><label className="form-label" htmlFor="reg-confirm">Confirm Password</label><input
    id="reg-confirm"
    type="password"
    className="input-field"
    ...
    required
    style={{
      borderColor: confirmPassword && confirmPassword !== password ? "#ff4757" : confirmPassword && confirmPassword === password ? "#00ff87" : void 0
    }}
  /></div>{error && <div className="error-alert animate-fadeIn" role="alert"><span>⚠</span> {error}</div>}<button
    id="register-submit"
    type="submit"
    className="btn-primary w-full"
    disabled={isLoading}
  >{isLoading ? <><div className="spinner" /> Creating Account…</> : <><span>🚀</span> Launch into Arena</>}</button></form><div className="auth-footer"><p>
              Already have an account?{" "}<Link to="/login" className="auth-link" id="go-to-login">
                Sign In →
              </Link></p></div></div></div><style>{`
        .auth-page {
          min-height: 100vh;
          display: flex;
          align-items: center;
